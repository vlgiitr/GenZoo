...
All dependencies required by this repo can be downloaded by creating a virtual environment and running
...
+ `-load_path` requires the path to directory containing the training checkpoints. The latest checkpoint is loaded from the directory.
+ `-grid_size` requires integer `n` and will generate n*n images in a grid.
+ `-save_path` requires the path to directory where the generated images will be saved. If the directory doesn't exist the script will create it.

To generate images from pre-trained model download checkpoint files from the Google Drive link from `model.txt` file.
...
1. **configs**: This folder contains config.ini files used to specifying hyperparameters used in experiments.
...
#### MNIST
Discriminator Loss
![disc_loss](resources/mnist_disc_loss.png)

Generator Loss
![gen_loss](resources/mnist_gen_loss.png)

#### CIFAR-10
Discriminator Loss
![disc_loss](resources/cifar_disc_loss.png)

Generator Loss
![gen_loss](resources/cifar_gen_loss.png)
...
### CIFAR-10
Training on the CIFAR-10 dataset was challenging. The dataset was varied and the network has higher number of parameters to train. This resulted in training times of upto 1.5 min/epoch on Google Colab. The model was trained for 200 epochs and produced images which were similar to the dataset.
